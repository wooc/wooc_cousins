...
//
namespace Wooc\WebtreesAddon\WoocCousinsModule;
...
class WoocCousinsModule extends AbstractModule implements ModuleTabInterface {
	
	public function __construct() {
		parent::__construct('wooc_cousins');
	}

	// Extend Module
	public function getTitle() {
		return /* I18N: Name of a module/tab on the individual page. */ I18N::translate('Wooc Cousins');
	}

	public function getTabTitle() {
		return /* I18N: Title used in the tab panel */ I18N::translate('Cousins');
	}
	...
	public function getTabContent() {
		...
		$html = '<style type="text/css">
				#cousins_tab {width:98%;}
				#cousins_tab h3 {margin:10px 20px;}
				#cousins_tab h4 {font-size:16px;font-weight:900;margin:0 0 5px;text-align:center;}
				#cousins_tab h5 {margin:0;clear:both;padding:10px 0 2px 0;}
				#cousins_tab_content {border-spacing:10px;display:inline-table;margin:0 10px;overflow:hidden;padding:0;width:98%;}
		...
		foreach ($list_f3 as $id3) {
			$i++;
			$record = Individual::getInstance($id3, $WT_TREE);
			if ($record->getPrimaryChildFamily()) {
				...
				$family = Family::getInstance($cousinParentFamily, $WT_TREE);
				...
				if (isset($pedi) && $pedi != 'birth') {
					$label = '<span class="cousins_pedi">' . GedcomCode\GedcomCodePedi::getValue($pedi, $record) . '</span>';
				}
